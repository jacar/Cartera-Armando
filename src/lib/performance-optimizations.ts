...
export function optimizeResourceLoading(): void {
  ...
  if ('requestIdleCallback' in window) {
    ...
    const deferredScripts = Array.from(document.querySelectorAll('script[data-defer="true"]'));
  ...
  if ('IntersectionObserver' in window) {
    const handleIntersection = (entries: IntersectionObserverEntry[], observer: IntersectionObserver) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          } else if (element instanceof HTMLElement && element.dataset.backgroundSrc) {
    ...
    document.querySelectorAll('[data-src], [data-background-src]').forEach(el => {
...
export function optimizeInitialRender(): void {
  ...
  importantDomains.forEach(domain => {
    const linkExists = document.querySelector(`link[rel="preconnect"][href="${domain}"]`);
    if (!linkExists) {
...
export function optimizeReactRendering(): void {
  ...
  document.querySelectorAll('.scroll-container, .scroll-view').forEach(container => {
...
export function detectMemoryLeaks(): void {
  ...
  if ('memory' in window.performance) {
    ...
    if (memoryInfo) {
      ...
      if (memoryInfo.usedJSHeapSize > memoryThreshold) {
        console.warn('⚠️ Alto uso de memoria detectado. Ejecutando limpieza de emergencia.');
  ...
  const checkFrameRate = () => {
    ...
    if (frameDuration > 33) {
      ...
      if (frameDrops > 5) {
        console.warn('⚠️ Rendimiento bajo detectado. Aplicando optimizaciones de emergencia.');
